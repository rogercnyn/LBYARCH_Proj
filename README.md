...
## Demonstration
You can watch a demonstration of the program in the following video: [Youtube Link](https://youtu.be/lhRWVXmZAYs)

The video provides a walkthrough of the code and a brief explanation of its functionality. You may skip to the 6-minute marker to go directly to the program testing. 
...
2. **Conversion Process:**  
   - The program calls a function written in assembly (`imgCvtGrayDoubleToInt`) for each pixel. The function performs the following steps:  
     ...
     - Clamps the value to ensure it stays within the range `[0, 255]`.  Input values less than 0 will be mapped to 0 and those greater than 1 will be mapped to 255.
...
The table below summarizes the recorded execution times for different image sizes across 30 runs of randomly generated input matrices. The average time for each image size is also provided.
...
The grayscale pixel converter is a straightforward yet powerful demonstration of how C seamlessly interfaces with assembly language. This reflects the deep relationship between the two languages, as C is closely rooted in assembly, offering both high-level abstraction and low-level control. By using C for input, output, and performance tracking, and assembly for the core pixel conversion, the program showcases how these languages work together to achieve high efficiency and simplicity in image processing.
